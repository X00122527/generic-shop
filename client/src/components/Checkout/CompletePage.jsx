...
export default function CompletePage() {
  ...
  return (
    <div className="payment-status">
      <div className="status-icon" style={{backgroundColor: STATUS_CONTENT_MAP[status].iconColor}}>
        {STATUS_CONTENT_MAP[status].icon}
      </div>
      <h2 className="status-text">{STATUS_CONTENT_MAP[status].text}</h2>
      {intentId && <div className="details-table">
        <table>
          <tbody>
            <tr>
      ...
      {intentId && <a href={`https://dashboard.stripe.com/payments/${intentId}`} className="view-details" rel="noopener noreferrer" target="_blank">View details
      ...
      <a className="retry-button" href="/checkout">Test another</a>
    </div>
  );
}