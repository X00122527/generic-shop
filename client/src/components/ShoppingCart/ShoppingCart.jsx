...
function ShoppingCart() {
    ...
    const handleItemCountChange = (data) => {
        const updatedCart = [...cart];
        console.log('handleItemCountChange', data);
        if ('message' in data) {
            console.log('actually deleting item')
            const itemIndex = updatedCart.findIndex((item) => item.id === Number(data.id));
            console.log('index to delete: ',itemIndex);
            if (itemIndex !== -1) { 
                updatedCart.splice(itemIndex, 1);
            }
        }
        else {
            console.log('just updating item')
        ...
        const deleteItem = (itemId) => {
            ...
            fetch(url, options)
                ...
                .then(data => {
                    console.log(data);
                    handleItemCountChange({ message: "deleted", id: itemId })
